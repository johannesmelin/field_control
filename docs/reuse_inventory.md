...
| Vändgeometri och A4-positionering | `oak_d_sr_navigation/src/navigation/state_machine.py` | Återanvänd den verifierade modellen: rena signed hjulvinklar, färsk 0x92-baslinje, A4 absoluta motorlägen och per-hjul-hastighetsförhållande. 8:1 och fysiskt tecken appliceras exakt en gång i CAN-workern. `in_row_turn_wheel_degrees=720` är fortsatt en okalibrerad startpunkt. |
...
| V3.8 CAN, 0xA2/0xA4/0x81/0x9C/0x92, tidsgränser | `remote_control/remote_control/physical.py` | Återanvänd som ensam SocketCAN-worker med verifierad same-ID-profil, bounded deadlines, STOP+0x9C-settle och atomära 0x92-par. A4 har färsk baslinje, 1,5 ms motorlucka, målbekräftelse och STOP-preemption. |
