        ...
        class Handler(BaseHTTPRequestHandler):
            def do_GET(self) -> None:
                ...
                if path in ("/stream/raw", "/stream/overlay", "/stream/buds", "/stream/leaves", "/stream/marker",
                            "/stream/cam2-raw", "/stream/cam2-buds", "/stream/cam2-leaves"):
                ...
                if path in ("/snapshot/raw", "/snapshot/overlay", "/snapshot/buds", "/snapshot/leaves", "/snapshot/marker",
                            "/snapshot/cam2-raw", "/snapshot/cam2-buds", "/snapshot/cam2-leaves"):
...
<section class="panel" style="margin-top:16px"><h2>Live views</h2><div class="streams"><div><h3>Cam 1 Original</h3><div class="legend" aria-label="Cam 1 zone colours"><span><i class="swatch" style="background:#00b4ff"></i>Navigation row 1</span><span><i class="swatch" style="background:#ffff00"></i>Trigger row 1</span><span><i class="swatch" style="background:#ff00ff"></i>Pick row 1</span><span><i class="swatch" style="background:#ffffb4"></i>Navigation row 2</span><span><i class="swatch" style="background:#b4b400"></i>Trigger row 2</span><span><i class="swatch" style="background:#ff00b4"></i>Pick row 2</span><span><i class="swatch" style="background:#ffb400"></i>Turn marker</span><span><i class="swatch" style="background:#ff0000"></i>x goal</span></div><img data-snapshot-view="raw" alt="Cam 1 original"><div class="camera-masks"><div><h4>Buds mask</h4><img data-snapshot-view="buds" alt="Cam 1 buds mask"></div><div><h4>Leaves mask</h4><img data-snapshot-view="leaves" alt="Cam 1 leaves mask"></div></div></div><div><h3>Cam 2 Original</h3><div class="legend" aria-label="Cam 2 zone colours"><span><i class="swatch" style="background:#00b4ff"></i>Navigation row 3</span><span><i class="swatch" style="background:#ffff00"></i>Trigger row 3</span><span><i class="swatch" style="background:#ff00ff"></i>Pick row 3</span><span><i class="swatch" style="background:#ffffb4"></i>Navigation row 4</span><span><i class="swatch" style="background:#b4b400"></i>Trigger row 4</span><span><i class="swatch" style="background:#ff00b4"></i>Pick row 4</span><span><i class="swatch" style="background:#ffb400"></i>Turn marker</span><span><i class="swatch" style="background:#ff0000"></i>x goal</span></div><img data-snapshot-view="cam2-raw" alt="Cam 2 original"><div class="camera-masks"><div><h4>Buds mask</h4><img data-snapshot-view="cam2-buds" alt="Cam 2 buds mask"></div><div><h4>Leaves mask</h4><img data-snapshot-view="cam2-leaves" alt="Cam 2 leaves mask"></div></div></div></div></section></main>
<script>
...
document.head.insertAdjacentHTML('beforeend','<style>.camera-masks{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:7px}.camera-masks h4{margin:0 0 3px;font-size:11px}.camera-masks img{width:100%;max-height:110px;object-fit:contain}@media(min-width:1181px){.panel[aria-label="Control"] .mode-actions button,.panel[aria-label="Control"] .manual-controls button{min-height:38px;padding:7px 9px;font-size:12px}.panel[aria-label="Control"] .speed-input{margin:2px 0 7px;padding:5px;font-size:12px}.streams{grid-template-columns:1fr 1fr}.streams>div>img{max-height:220px}}</style>');
...
function configLabel(path){const match=path.match(/^vision\\.(navigation|trigger|pick)_zone(?:_([2-4]))?\\.x_distance$/);if(match)return `${match[1]} ${match[2]||'1'} x distance from x goal`;if(path==='vision.x_goal')return 'x_goal_1';if(/^vision\\.x_goal_[2-4]$/.test(path))return path.slice(7);if(/^vision\\.cam_[12]_serial_number$/.test(path))return 'Camera serial number';if(/^vision\\.row_[1-4]_enabled$/.test(path))return 'Enabled';return isRpmPath(path)?`${path} (wheel RPM)`:path;}
const configGroups=[
...
function configGroupForPath(path){
  if(/^vision\\.(camera_serial_number|cam_[12]_serial_number|row_[1-4]_enabled|x_goal(?:_[2-4])?|navigation_zone(?:_[2-4])?|trigger_zone(?:_[2-4])?|pick_zone(?:_[2-4])?|turn_marker_zone)/.test(path))return 'rows';
...
function rowZoneLabel(path){const zone=path.match(/^vision\.(navigation|pick|trigger)_zone(?:_([2-4]))?\.(.+)$/);if(zone)return `${zone[3]}_row_${zone[2]||'1'}`;const leaf=path.split('.').at(-1);return leaf==='x_goal'?'x_goal_1':leaf==='x_goal_2'?'x_goal_2':leaf.endsWith('_enabled')?'Enabled':leaf;}
...
const renderConfigFourRowsBase=renderConfig;
renderConfig=candidate=>{
  renderConfigFourRowsBase(candidate);
  const section=document.querySelector('.config-section[data-config-group="rows"]'),grid=section?.querySelector('.row-zone-grid');
  if(!section||!grid)return;
  section.querySelector('.row-camera-serial')?.remove();
  const columns=[...grid.querySelectorAll('.row-zone-column')];
  for(const row of [3,4]){
    const column=document.createElement('section'),heading=document.createElement('h4'),goal=document.createElement('div');
    column.className='row-zone-column';heading.textContent=`Rad ${row}`;goal.className='row-zone-goal';column.append(heading,goal);
    for(const [kind,title] of [['navigation','navigation_boundaries'],['pick','pick_boundaries'],['trigger','trigger_boundaries']]){
      const zone=document.createElement('section'),zoneHeading=document.createElement('h5'),inputs=document.createElement('div');
      zone.className='row-zone-group';zoneHeading.textContent=title;inputs.className='row-zone-inputs';
      for(const input of document.querySelectorAll(`input[data-path^="vision.${kind}_zone_${row}."]`)){
        const label=input.parentElement;
        label.textContent=rowZoneLabel(input.dataset.path);
        label.append(input);
        inputs.append(label);
      }
      zone.append(zoneHeading,inputs);column.append(zone);
    }
    grid.append(column);columns.push(column);
  }
  const cameraLabels={1:document.querySelector('input[data-path="vision.cam_1_serial_number"]')?.parentElement,3:document.querySelector('input[data-path="vision.cam_2_serial_number"]')?.parentElement};
  for(let row=1;row<=4;row++){
    const column=columns[row-1],heading=column?.querySelector('h4'),goal=column?.querySelector('.row-zone-goal');
    if(!column||!heading||!goal)continue;
    for(const zone of column.querySelectorAll('.row-zone-group')){
      const input=zone.querySelector('input[data-path]'),kind=input?.dataset.path.match(/^vision\.(navigation|pick|trigger)_zone/)?.[1],zoneHeading=zone.querySelector('h5');
      if(kind&&zoneHeading)zoneHeading.textContent=`${kind}_boundaries_row_${row}`;
    }
    const cameraColumn=document.createElement('div'),camera=cameraLabels[row]?document.createElement('div'):document.createElement('div');
    cameraColumn.className='row-camera-column';
    camera.className=cameraLabels[row]?'row-camera-binding':'row-camera-spacer';
    if(cameraLabels[row]){
      const title=document.createElement('strong');title.textContent=`Cam ${row===1?1:2}`;
      camera.append(title,cameraLabels[row]);
    }
    column.replaceWith(cameraColumn);cameraColumn.append(camera,column);
    const enabled=document.querySelector(`input[data-path="vision.row_${row}_enabled"]`)?.parentElement;
    if(enabled){enabled.classList.add('row-enabled');heading.after(enabled);}
    const goalPath=row===1?'vision.x_goal':`vision.x_goal_${row}`,goalLabel=document.querySelector(`input[data-path="${goalPath}"]`)?.parentElement;
    if(goalLabel)goal.append(goalLabel);
  }
}
