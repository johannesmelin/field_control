...
function configLabel(path){const match=path.match(/^vision\\.(navigation|trigger|pick)_zone(_2)?\\.x_distance$/);if(match)return `${match[1]}${match[2]?' 2':' 1'} x distance from x goal`;if(path==='vision.x_goal_2')return 'x goal 2';return isRpmPath(path)?`${path} (wheel RPM)`:path;}
...
function renderConfig(candidate){candidate=migrateCentredLegacyZones(candidate);profileCandidate=candidate;const ratio=Number(candidate.odometry_geometry?.motor_turns_per_wheel_turn);gearRatio=Number.isFinite(ratio)&&ratio>0?ratio:null;document.querySelectorAll('[data-direct]').forEach(el=>{const value=atPath(candidate,el.dataset.direct);if(el.type==='checkbox')el.checked=Boolean(value);else el.value=value??'';});const root=document.getElementById('config-fields');root.textContent='';for(const [path,value] of leafEntries(candidate)){if(directPaths.includes(path))continue;const label=document.createElement('label');label.textContent=configLabel(path);const input=document.createElement('input');input.dataset.path=path;input.value=value===null?'':String(isRpmPath(path)?Number(value)/gearRatio:value);input.type=typeof value==='boolean'?'checkbox':'text';if(input.type==='checkbox')input.checked=value;label.append(input);root.append(label);}if(manualSpeedStatus)configureManualSpeed(manualSpeedStatus);}
