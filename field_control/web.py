        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def do_POST(self) -> None:
                ...
                if path in ("/api/config/save", "/api/config/select", "/api/config/restart"):
                    ...
                    if path != "/api/config/restart" and not self._configuration_safe():
                        self._conflict("konfiguration kan endast ändras i MANUAL utan aktiv lease och med disarmerad motorutgång"); return
                    try:
                        ...
                        if path == "/api/config/restart":
                            ...
                            if not restart_reserved:
                                raise ValueError("konfiguration kan endast ändras i MANUAL utan aktiv lease och med disarmerad motorutgång")
                            name = save_profile(config, profiles_dir)
